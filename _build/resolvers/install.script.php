...
/**
 * Description: Resolver script for pdfparser package
 * @package pdfparser
 * @subpackage build
 */
    ...
    case xPDOTransport::ACTION_INSTALL:
        ...
        foreach ($settings as $key) {
            if (isset($options[$key])) {
                $setting = $object->xpdo->getObject('modSystemSetting', array('key' => 'pdfparser.' . $key));
                if ($setting != null) {
                    $setting->set('value', $options[$key]);
                    $setting->save();
                } else {
                    $object->xpdo->log(xPDO::LOG_LEVEL_ERROR, '[pdfparser] ' . $key . ' setting could not be found, so the setting could not be changed.');
                }
            }
        }
